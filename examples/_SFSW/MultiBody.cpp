...
void MultiBody::initPhysics() {
    ...
    btSphereShape *headShape = new btSphereShape(btScalar(1));
    ...
    btBoxShape *torsoShape = new btBoxShape(btVector3(1.5, 3, 1.5));
    ...
    btBoxShape *armShape = new btBoxShape(btVector3(0.5, 0.2, 0.5));
    m_collisionShapes.push_back(armShape);
    btTransform armTransform;
    armTransform.setIdentity();
    for (int i = 0; i < 10; i++) {
        btScalar mass(1);
        armTransform.setOrigin(btVector3(0, 19.8 - i * 0.4, 2));
    ...
    btBoxShape *legShape = new btBoxShape(btVector3(0.5, 0.2, 0.5));
    m_collisionShapes.push_back(legShape);
    btTransform legTransform;
    legTransform.setIdentity();
    for (int i = 0; i < 10; i++) {
        btScalar mass(1);
        legTransform.setOrigin(btVector3(0, 12.8 - i * 0.4, 2));
    ...
    btPoint2PointConstraint *headTorso = new btPoint2PointConstraint(*bodies[0], *bodies[1], btVector3(0, -1, 0), btVector3(0, 3, 0));
    m_dynamicsWorld->addConstraint(headTorso);

    btPoint2PointConstraint *torsoArm1 = new btPoint2PointConstraint(*bodies[1], *bodies[2], btVector3(0, 3, 1.1), btVector3(0, 0, -0.6));
    m_dynamicsWorld->addConstraint(torsoArm1);
    for (int i = 0; i < 9; i++) {
        btPoint2PointConstraint *armArm = new btPoint2PointConstraint(*bodies[2 + i * 2], *bodies[2 + i * 2 + 2], btVector3(0, -0.2, 0), btVector3(0, 0.2, 0));
        m_dynamicsWorld->addConstraint(armArm);
    }

    btPoint2PointConstraint *torsoArm2 = new btPoint2PointConstraint(*bodies[1], *bodies[3], btVector3(0, 3, -1.1), btVector3(0, 0, 0.6));
    m_dynamicsWorld->addConstraint(torsoArm2);
    for (int i = 0; i < 9; i++) {
        btPoint2PointConstraint *armArm = new btPoint2PointConstraint(*bodies[3 + i * 2], *bodies[3 + i * 2 + 2], btVector3(0, -0.2, 0), btVector3(0, 0.2, 0));
        m_dynamicsWorld->addConstraint(armArm);
    }

    btPoint2PointConstraint *torsoLeg1 = new btPoint2PointConstraint(*bodies[1], *bodies[22], btVector3(0, -3, 1), btVector3(0, 0.2, 0));
    m_dynamicsWorld->addConstraint(torsoLeg1);
    for (int i = 0; i < 9; i++) {
        btPoint2PointConstraint *legLeg = new btPoint2PointConstraint(*bodies[22 + i * 2], *bodies[22 + i * 2 + 2], btVector3(0, -0.2, 0), btVector3(0, 0.2, 0));
        m_dynamicsWorld->addConstraint(legLeg);
    }

    btPoint2PointConstraint *torsoLeg2 = new btPoint2PointConstraint(*bodies[1], *bodies[23], btVector3(0, -3, -1), btVector3(0, 0.2, 0));
    m_dynamicsWorld->addConstraint(torsoLeg2);
    for (int i = 0; i < 9; i++) {
        btPoint2PointConstraint *legLeg = new btPoint2PointConstraint(*bodies[23 + i * 2], *bodies[23 + i * 2 + 2], btVector3(0, -0.2, 0), btVector3(0, 0.2, 0));
        m_dynamicsWorld->addConstraint(legLeg);
    }
